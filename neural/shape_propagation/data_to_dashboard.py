...
# Load configuration (e.g., from config.yaml) and environment variables
config = {}
try:
...
# Resolve credentials from environment first, then config
env_username = os.getenv("NEURAL_AUTH_USERNAME")
env_password = os.getenv("NEURAL_AUTH_PASSWORD")
cfg_auth = (config.get("auth") or {})
username = env_username or cfg_auth.get("username") or "admin"
password = env_password or cfg_auth.get("password") or ""

users = {username: generate_password_hash(password)} if password else {}
auth = HTTPBasicAuth()
...
def send_trace_update():
    ...
    while True:
        ...
        update_interval = (
            int(os.getenv("NEURAL_WS_INTERVAL", "0")) or
            int((config.get("websocket_interval") or 1000))
        )
