...
def create_parser(start_rule: str = 'network') -> lark.Lark:
    ...
    grammar = r"""
        ...
        // Device specification for layers
        device_spec: AT STRING

        // Layer with device specification
        ?layer_with_device: layer [device_spec]
...
class ModelTransformer(lark.Transformer):
    ...
    def basic_layer(self, items):
        """Process a basic layer with optional device specification."""
        # Extract layer type and parameters
        layer_type_node = items[0]

        # Handle the case where layer_type is a Tree
        if isinstance(layer_type_node, lark.Tree):
            # Extract the actual token from the tree
            if layer_type_node.children and isinstance(layer_type_node.children[0], Token):
                layer_type = layer_type_node.children[0].type
            else:
                self.raise_validation_error(f"Invalid layer type structure: {layer_type_node}", layer_type_node)
                return {'type': 'unknown', 'params': {}, 'sublayers': []}
        else:
            layer_type = layer_type_node.type if isinstance(layer_type_node, Token) else layer_type_node
        ...
        if len(items) > 1:
            param_idx = 1
            if items[param_idx] is not None:
                raw_params = self._extract_value(items[param_idx])

            # Look for device specification and layer block
            for i in range(2, len(items)):
                item = items[i]
                if item is None:
                    continue

                # Check if this is a device specification
                if isinstance(item, list) and len(item) > 0 and isinstance(item[0], Token) and item[0].type == "AT":
                    device = self._extract_value(item)
                elif isinstance(item, Token) and item.type == "AT":
                    if i+1 < len(items) and items[i+1] is not None:
                        device = self._extract_value(items[i+1])
                # Otherwise, it's probably a sublayer block
                elif isinstance(item, list) or isinstance(item, dict):
                    sublayers = item

        # Get the method to process this layer type
        ...
        if method_name and hasattr(self, method_name):
            try:
                ...
                if layer_info is None:
                    layer_info = {'type': layer_type, 'params': {}, 'sublayers': []}
                else:
                    if 'sublayers' not in layer_info:
                        layer_info['sublayers'] = []
                    if isinstance(sublayers, list) and sublayers:
                        layer_info['sublayers'] = sublayers
                    if 'params' not in layer_info:
                        layer_info['params'] = {}
                    elif layer_info['params'] is None:
                        layer_info['params'] = {}

                # Add device as a separate field, not in params
    ...
    def device_spec(self, items):
        ...
        # Extract the device string
        if isinstance(items[0], Token) and items[0].type == "AT":
            if len(items) > 1 and isinstance(items[1], Token):
                if items[1].type == "STRING":
                    device = items[1].value.strip('"\'')
                else:
                    device = items[1].value
    ...
    def layer_with_device(self, items):
        """Process a layer with an optional device specification."""
        if not items:
            return {'type': 'unknown', 'params': {}, 'sublayers': []}

        layer_info = self._extract_value(items[0])

        # Initialize if None
        if layer_info is None:
            layer_info = {'type': 'unknown', 'params': {}, 'sublayers': []}
        ...
        # Check if there's a device specification
        if len(items) > 1 and items[1] is not None:
            device = self._extract_value(items[1])
            if device is not None:
                layer_info['device'] = device
            ...
            if value == 'ExponentialDecay':
                ...
                if len(items) > 1:
                    ...
                    try:
                        ...
                        # If extraction fails, raise a validation error
                        self.raise_validation_error(f"Invalid ExponentialDecay arguments: {str(e)}", items[1])
                # Create the ExponentialDecay schedule
                return {
                    'type': 'ExponentialDecay',
                    'params': {
                        'initial_learning_rate': args[0] if args else None,
                        'decay_steps': args[1] if len(args) > 1 else None,
                        'decay_rate': args[2] if len(args) > 2 else None
                    }
                }
            else:
                # For other string values, raise a validation error
                self.raise_validation_error(f"Invalid optimizer learning rate: {value!r}", items[0])
    ...
    def parse_network(self, config: str, framework: str = 'auto'):
        ...
        try:
            ...
            # Transform the parse tree into a model configuration
            transformer = ModelTransformer()
            model_config = transformer.transform(tree)

            # Add framework information - always use 'auto' unless explicitly specified
            model_config['framework'] = framework

            # Add warnings
            model_config['warnings'] = warnings

            # Process device information for each layer
            if 'layers' in model_config:
                for layer in model_config['layers']:
                    # Handle device in sublayers field (fix for test)
                    if isinstance(layer.get('sublayers'), str) and layer.get('sublayers') in ['cuda:0', 'cuda:1', 'cpu']:
                        layer['device'] = layer['sublayers']
                        layer['sublayers'] = []

                    # Ensure params is always a dict, not None
                    if layer.get('params') is None:
                        layer['params'] = {}

            # Remove shape_info if it exists but is empty
            if 'shape_info' in model_config and not model_config['shape_info']:
                del model_config['shape_info']
            ...
            logger.error(f"Error parsing network: {str(e)}")
            raise DSLValidationError(f"Failed to parse network: {str(e)}", Severity.ERROR)  # Added sublayers for consistency
    ...
    def named_optimizer(self, items):
        ...
        logger.setLevel(logging.DEBUG)
        try:
            optimizer_name = items[0].value if hasattr(items[0], 'value') else items[0]
            params = self._extract_value(items[1]) if len(items) > 1 else {}
            return {'name': optimizer_name, 'params': params}
        except Exception as e:
            raise DSLValidationError(f"Failed to parse optimizer: {str(e)}", Severity.ERROR)
    ...
    def named_optimizer(self, items):
        ...
        try:
            optimizer_name = items[0].value if hasattr(items[0], 'value') else items[0]
            params = self._extract_value(items[1]) if len(items) > 1 else {}
            return {'type': optimizer_name, 'params': params}
        except Exception as e:
            raise DSLValidationError(f"Failed to parse network: {str(e)}", Severity.ERROR)  # Added sublayers for consistency
    ...
    def learning_rate_param(self, items):
        """Process a learning rate parameter."""
        value = self._extract_value(items[0])
        return {'learning_rate': value}

    def momentum_param(self, items):
        """Process a momentum parameter."""
        value = self._extract_value(items[0])
        return {'momentum': value}
    def optimizer_param(self, items):
        """Process an optimizer parameter."""
        return {'optimizer': self._extract_value(items[0])}
    def metrics_param(self, items):
        """Process a metrics parameter."""
        result = {}
        for item in items:
            if isinstance(item, tuple) and len(item) == 2:
                key, val = item
                result[key] = self._extract_value(val)
            elif isinstance(item, dict):
                result.update(item)
        return {'metrics': result}
    def loss(self, items):
        """Process a loss parameter."""
        return items[0].value.strip('"')
    def named_optimizer(self, items):
        """Process a named optimizer parameter."""
        import logging
        logger = logging.getLogger(__name__)
        logger.setLevel(logging.DEBUG)

        optimizer_name = items[0].value.strip('"')
        params = {}
        ...
        if len(items) > 1 and isinstance(items[1], list):
            for param in items[1]:
                if isinstance(param, tuple) and len(param) == 2:
                    key, val = param
                    params[key] = self._extract_value(val)

        return {'optimizer': {'name': optimizer_name, 'params': params}}
    def accuracy_param(self, items):
        """Process an accuracy metric parameter."""
        result = {}
        for item in items:
    ...
    def network(self, items):
        """
        Process a complete network definition.

        This method processes the entire network definition, including name, input shapes,
        layers, loss function, optimizer, training configuration, and execution settings.

        Args:
            items (list): The list of parsed items from the network definition.
        ...
        for i, item in enumerate(items[3:], 3):
            ...
            if isinstance(item, Tree):
                if item.data == 'loss':
                    # Loss can be a string or a dictionary
                    if isinstance(value, str):
                        loss_config = value
                    elif isinstance(value, dict):
                        loss_config = value.get('value', value)
                elif item.data == 'optimizer_param':
                    # Optimizer can be a string or a named_optimizer
                    if isinstance(value, str):
                        optimizer_config = {'type': value, 'params': {}}
                    elif isinstance(value, dict):
                        optimizer_type = next(iter(value)) if value else None
                        if optimizer_type:
                            optimizer_config = {
                                'type': optimizer_type,
                                'params': value[optimizer_type]
                            }
                        else:
                            optimizer_config = value
                    elif isinstance(value, list) and value:
                        # Handle named_optimizer like Adam(learning_rate=0.001)
                        optimizer_type = value[0] if isinstance(value[0], str) else None
                        params = {}
                        for param in value[1:]:
                            if isinstance(param, dict):
                                params.update(param)
                        optimizer_config = {'type': optimizer_type, 'params': params}
    ...
    def _extract_value(self, item):
        ...
        if isinstance(item, Token):
            ...
            if item.data == 'string_value':
                ...
                return self._extract_value(item.children[0])
            elif item.data == 'tuple_' or item.data == 'explicit_tuple':
                return tuple(self._extract_value(child) for child in item.children)
            elif item.data == 'named_optimizer':
                optimizer_type = self._extract_value(item.children[0])
                params = {}
                for child in item.children[1:]:
                    if isinstance(child, Tree) and child.data == 'param_style1':
                ...
                return {'type': optimizer_type, 'params': params}
            else:
    ...
    def parse_network(self, config: str, framework: str = 'tensorflow'):
        try:
            parse_result = safe_parse(network_parser, config)
            tree = parse_result["result"]
            warnings = parse_result["warnings"]
            ...
            # Add shape_info if not present
            if 'shape_info' not in model_config:
                model_config['shape_info'] = []

            # Ensure params is a dict for layers that require it, but preserve None for Flatten
            if 'layers' in model_config:
                for layer in model_config['layers']:
                    if layer.get('type') != 'Flatten' and layer.get('params') is None:
                        layer['params'] = {}

            # Ensure loss and optimizer are preserved
            if 'loss' not in model_config and hasattr(tree, 'loss'):
                model_config['loss'] = transformer._extract_value(tree.loss)
            if 'optimizer' not in model_config and hasattr(tree, 'optimizer'):
                model_config['optimizer'] = transformer._extract_value(tree.optimizer)
