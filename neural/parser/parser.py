    ...
    def network(self, items):
        ...
        for i, item in enumerate(items[3:], 3):
            ...
            if isinstance(value, dict):
                if 'type' in value and value.get('type') == 'loss':
                    loss_config = value
                elif 'type' in value and value.get('type') in {'Adam', 'SGD', 'RMSprop', 'Adagrad', 'Adadelta', 'Adamax', 'Nadam'}:
                    optimizer_config = value
                elif 'type' in value and value.get('type') == 'training_config':
                    training_config = value.get('params', {})
                elif 'device' in value:  # This is likely an execution_config
